...
from tools import get_rendered_html, download_file, post_request, run_code, add_dependencies, get_request, transcribe_audio, analyze_image, create_bar_chart, extract_lsb_message
...
TOOLS = [run_code, get_rendered_html, download_file, post_request, add_dependencies, get_request, transcribe_audio, analyze_image, create_bar_chart, extract_lsb_message]
...
rate_limiter = InMemoryRateLimiter(
    requests_per_second=8/60,  # 8 requests per minute (below 10 limit)
    check_every_n_seconds=1,  
    max_bucket_size=8  
)
llm = init_chat_model(
   model_provider="google_genai",
   model="gemini-2.0-flash",  # Use 2.0-flash which has higher limits
